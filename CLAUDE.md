...
**Commit proven, task-owned work as you go.** This heading is not a blanket ban on ordinary
commits: it means the prohibited categories below must never be staged or committed. Once a
focused change has its required proof, inspect the exact staged paths and make the normal focused
...
2. **No development diaries in the repo.** `docs/binder/**`, session reports, evidence
   registries, handoffs, live-test ledgers, campaign notes — ALL of it goes to the maintainer's
   external Latitude notes tree, never into the repository. If you are writing a dated narrative
   of what you did, you are writing a notes file, not a repo file.
