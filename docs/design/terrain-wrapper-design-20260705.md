...
**Status:** LOCKED design, pre-implementation. No code written yet. All MC-26.2 API facts below were verified this change by reading the actual `minecraft-merged-deobf-26.2.jar` and the live repo source (see the Verified-facts and Pre-implementation-verification sections). Items that genuinely cannot be settled without touching code / running the live pass at implementation time are called out explicitly as **verification items** or **accepted residual risks**, not guessed and not silently dropped.
...
## 0. Verified MC-26.2 facts (ground truth, read from the dependency jar + repo this change)
...
Repo facts re-verified this change:
...
- **R8 — CLOSED 2026-07-06 (found live, not by any headless proof).** The install gate's `instanceof GeoAuthorityProvider` check (added for the seed-0 defense, sweeper finding #7) ran once at `ChunkMap`-construction time, before `GlobeMod`'s create-world flow finished rebuilding `GEO_V2_PROVIDER` — so it always saw the still-uninitialized placeholder and the wrapper never installed on ANY freshly created world, any seed, any strength. Neither the mechanical proof gate nor the 6-lens sweeper audit could have caught this: both exercise `installIfArmed` only via the dev/atlas path, whose `RandomState` is always built on an already-fully-loaded `ServerLevel`, so the exact ordering hazard structurally cannot occur there. **Fix:** the realness check moved from the one-time install gate into `GeoTerrainBiasFunction.compute()`, re-evaluated per call instead of snapshotted once — see that class's javadoc and `LESSONS.md` L18 (main worktree). `InstallResult.SKIPPED_NOOP_PROVIDER` was removed; `INSTALLED` now means "structurally wrapped," not "actively biasing this call."
- **R9 — CLOSED 2026-07-06 (found live, not by any headless proof).** Once R8's fix let the wrapper actually install, chunk generation slowed noticeably at any nonzero strength: `GeoAuthority.sample(x,z)` doesn't depend on Y, but `compute()` is invoked once per vertical noise-lattice cell (tens of times per column), redundantly repeating the same expensive sample every call. Invisible to the Phase 4 Spark baseline (captured at `S=0`, which never reaches this code). **Fix:** a per-thread, single-column last-value memo in `GeoTerrainBiasFunction` (value-only optimization, does not change any prior byte-identity result).
- **R4 status update (2026-07-06):** confirmed further via live testing, not just the headless finding. At `S=1.0` the vanilla floating-landmass result (R4's original finding) still holds; at `S=10.0` (a deliberate stress-test value, not a candidate for real use) the bias produces a dramatic, sheer rift/canyon at a coastline. Both confirm K=0.25 is too strong for any value near or above 1.0 on vanilla density — the real usable strength range is expected well under 1.0. Calibration is the open item as of this update; see the external record execution log's "Live pass findings" section.
