...
| --- | --- | --- | --- | --- |
| Loading pane covers only the last screen; flag cleared mid-load | `19323578` + `b75a5dac` | **N/A** | **N/A** | **PORT ARTIFACT, not a 1.5-wide bug — the shared code is identical but the save format is not.** The detector `NoiseBasedChunkGenerator.stable(key)` is `Holder.is(ResourceKey)`, true only for a *reference* holder. Measured across real saves: **every** 26.1.2 save (`Lat 1.5 - 26.1.2 - TEST`: 3 sampled) and **every** 26.2 save (`LATITUDE 26.2`: 10 sampled) stores the overworld generator settings as a **reference**, so `stable()` resolves and the flag is never cleared; only the 1.21.11 save (`test 21`) stores them as an **inline compound**, so the holder has no key and the check can never match. Maintainer confirmed empirically she has never seen the symptom on 26.1.2 (2026-08-09) — that observation is correct and takes precedence over the identical-source reading, which is what first suggested (wrongly) that all three lines were affected. **Do not backport.** The 26.x lines additionally lack `GenericMessageScreen`/`ProgressScreen` overlays entirely, so their chain coverage is narrower, but with the flag intact their `LevelLoadingScreen` still brands the phase that matters there. 1.21.11 fix maintainer-approved live GREEN 2026-08-09 (TEST 23, `2a8c7cfe`). |
...
(`bc6b1cf7`, the own-line picks 15 and 16, and the version-bump chore have no cherry-pickable
