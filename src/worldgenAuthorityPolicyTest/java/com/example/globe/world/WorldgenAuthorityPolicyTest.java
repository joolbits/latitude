    ...
    private static void structureLocateServiceIsBoundedAndTickDelivered() throws Exception {
        ...
        assertTrue(
                service.contains("showTeleportLocateResult(source, target, context.origin(), outcome.result())")
                        && service.contains("registerTeleportCommand(CommandDispatcher<CommandSourceStack> dispatcher)")
                ...
                "the coordinate click must use a player-bound one-time non-elevated teleport or a clear not-found message");
        assertTrue(
        ...
        assertTrue(
                ...
                "the one-time locate teleport action must be registered in the ordinary server command tree");
        assertTrue(
