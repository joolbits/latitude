...
public final class LatitudeBiomeSource extends BiomeSource {
     ...
     * tag-routed and ledger-routed alike. Real generation never calls this path for biome selection
     * ({@code ChunkGeneratorPopulateBiomesMixin} resolves against the full registry directly), so
     * leaving it unexpanded when {@code biomeRegistry == null} changes nothing about what
     * generates — only {@code /locate biome}'s candidate-membership gate, which previously declared
     * every custom biome unfindable before any search ran.
     */
    @Override
